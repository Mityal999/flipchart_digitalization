...
class ComplexShapeRecognition:
    # Класс для распознавания сложных форм на изображении
...
def shape_recognition(IMAGE_PATH):
    recognition = ComplexShapeRecognition(IMAGE_PATH)
...
def shape_digitalization(name, tmp_folder, xml_output_folder):
    xml_path = f"{xml_output_folder}/{name}_shapes.xml"
    IMAGE_PATH = f"{tmp_folder}/{name}_cut.jpg"
